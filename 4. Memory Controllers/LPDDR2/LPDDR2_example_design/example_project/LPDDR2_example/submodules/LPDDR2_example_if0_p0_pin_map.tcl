...
proc LPDDR2_example_if0_p0_get_ddr_pins { instname allpins } {
	...
	if {[string compare -nocase $pins(driver_core_ck_pins) ""] != 0 && [get_collection_size [get_registers -nowarn $pins(driver_core_ck_pins)]] > 0} {
		set pll_driver_core_clock_id [LPDDR2_example_if0_p0_get_output_clock_id $pins(driver_core_ck_pins) "DRIVER CORE CK" msg_list]
		if {$pll_driver_core_clock_id == -1} {
			foreach {msg_type msg} $msg_list {
				post_message -type $msg_type "LPDDR2_example_if0_p0_pin_map.tcl: $msg"
			}
			post_message -type critical_warning "LPDDR2_example_if0_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(driver_core_ck_pins)]"
		} else {
			set pll_driver_core_clock [LPDDR2_example_if0_p0_get_pll_clock_name $pll_driver_core_clock_id]
		}
	}
	...
	if {[get_collection_size [get_registers -nowarn $pins(avl_phy_ck_pins)]] > 0} {
		set pll_avl_phy_clock_id [LPDDR2_example_if0_p0_get_output_clock_id $pins(avl_phy_ck_pins) "Avalon PHY Bus CK" msg_list]
		if {$pll_avl_phy_clock_id == -1} {
			foreach {msg_type msg} $msg_list {
				post_message -type $msg_type "LPDDR2_example_if0_p0_pin_map.tcl: $msg"
			}
			post_message -type critical_warning "LPDDR2_example_if0_p0_pin_map.tcl: Failed to find PLL clock for pins [join $pins(avl_phy_ck_pins)]"
		} else {
			set pll_avl_phy_clock [LPDDR2_example_if0_p0_get_pll_clock_name $pll_avl_phy_clock_id]
		}
	}
...
proc LPDDR2_example_if0_p0_dump_static_pin_map { ddr_db_par filename } {
	...
	foreach instname $instnames {
		...
		LPDDR2_example_if0_p0_static_map_expand_string $FH pins pll_avl_phy_clock
		LPDDR2_example_if0_p0_static_map_expand_string $FH pins pll_config_clock
		LPDDR2_example_if0_p0_static_map_expand_string $FH pins pll_driver_core_clock
